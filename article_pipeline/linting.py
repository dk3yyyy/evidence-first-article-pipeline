...
def _sentences(text: str) -> list[tuple[str, int]]:
    ...
    for raw in re.split(r"(?<=[.!?])\s+|\n{2,}", reader):
        ...
        comparison = re.sub(r"[*_~]", "", normalized.casefold())
        comparison = re.sub(r"[.!?]+$", "", comparison).strip()
        out.append((comparison, len(words)))
    return out
...
def measure_distinctness(master: str, candidate: str, minimum_words: int = 12) -> dict[str, Any]:
    ...
    master_sentences = {
        sentence for sentence, count in _sentences(master) if count >= minimum_words
    }
    candidate_sentences = {
        sentence for sentence, count in _sentences(candidate) if count >= minimum_words
    }
...
def check_fabrication(
    text: str, terms: list[str] | tuple[str, ...] | None = None
) -> dict[str, Any]:
    ...
    for line_number, line in reader_lines(text):
        low = line.casefold()
